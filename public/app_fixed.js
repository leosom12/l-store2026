...
async function loadVersion() {
    try {
        ...
        if (versionElement && data.version) {
            versionElement.innerHTML = `<i class="ph ph-tag"></i> VersÃ£o: ${data.version}`;
        }
    } catch (error) {
...
function renderProductsTable(productsToRender) {
    ...
    productsToRender.forEach((product, index) => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <button onclick="editProduct(${product.id})" class="btn-icon">âœï¸</button>
                <button onclick="deleteProduct(${product.id})" class="btn-icon">ðŸ—‘ï¸</button>
            </td>
        `;
        tbody.appendChild(tr);
    });
        ...
        .then(boletos => {
            ...
            boletos.forEach(boleto => {
                ...
                if (boleto.status === 'pago') {
                    statusBadge = '<span class="status-badge active">âœ“ Pago</span>';
                    diasRestantes = '-';
                } else if (diffDays < 0) {
                    statusBadge = '<span class="status-badge inactive">âœ— Vencido</span>';
                    diasRestantes = `<span style="color: var(--accent-red);">${Math.abs(diffDays)} dias atrÃ¡s</span>`;
                } else if (diffDays <= 7) {
                    statusBadge = '<span class="status-badge warning">âš  Vencendo</span>';
                    diasRestantes = `<span style="color: var(--accent-orange);">${diffDays} dias</span>`;
                } else {
                    statusBadge = '<span class="status-badge">â³ Pendente</span>';
                ...
                tr.innerHTML = `
                    ...
                    <td>${diasRestantes}</td>
                    <td>
                        ${boleto.status !== 'pago' ? `<button onclick="markBoletoAsPaid(${boleto.id})" class="btn-sm btn-success" title="Marcar como Pago">âœ“ Pagar</button>` : ''}
                        <button onclick="editBoleto(${boleto.id})" class="btn-sm btn-secondary" title="Editar">âœŽ</button>
                        <button onclick="deleteBoleto(${boleto.id})" class="btn-sm btn-danger" title="Excluir">âœ—</button>
                    </td>
                `;
                tbody.appendChild(tr);
            });
    ...
    fetch('/api/config', {
        ...
        .then(data => {
            if (data.message) {
                ...
                alert('âŒ ' + (data.error || 'Erro ao salvar configuraÃ§Ã£o'));
            }
        })
        .catch(err => {
            console.error('Erro:', err);
...
function loadStoreConfig() {
    ...
    fetch('/api/config', {
        ...
        .catch(err => console.error('Erro ao carregar configuraÃ§Ãµes:', err));
}
...
function updateSellerPin() {
    ...
    fetch('/api/settings/pin', {
        ...
        .then(data => {
            ...
            alert('âœ… PIN atualizado com sucesso!');
        })
        .catch(err => {
...
function saveStoreConfig() {
    ...
    // For now, just alert
    alert('ConfiguraÃ§Ãµes salvas (SimulaÃ§Ã£o)!');
}
        ...
        card.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.8rem; padding-bottom: 0.8rem; border-bottom: 1px dashed #eee;">
                <div>
                    ...
                    <div style="font-size: 0.8rem; color: #6b7280;">Pedido #${order.id} â€¢ ${new Date(order.createdAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</div>
                </div>
...
function confirmOrder(orderId) {
    if (!confirm('Confirmar este pedido? O status serÃ¡ alterado para concluÃ­do.')) return;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Load store config
    if (typeof loadStoreConfig === 'function') {
        loadStoreConfig();
    }
});

// ==================== CONFIGURAÃ‡ÃƒO DA LOJA (FIX) ====================

function saveStoreConfig() {
    const input = document.getElementById('store-pix-key');
    if (!input) return;

    // Obter token (se houver auth)
    const token = localStorage.getItem('authToken');
    const storePixKey = input.value.trim();

    if (!storePixKey) {
        alert('Por favor, informe uma chave PIX vÃ¡lida.');
        return;
    }
    ...
    // Salvar no LocalStorage (Persistence)
    // Usamos 'storeConfig' como objeto principal e 'storePixKey' como backup
    localStorage.setItem('storeConfig', JSON.stringify(window.storeConfig));
    localStorage.setItem('storePixKey', storePixKey);

    console.log('âœ… Chave PIX salva localmente:', storePixKey);

    // Tentar salvar no Backend (opcional)
    if (token) {
        fetch('/api/config', {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                if (data.message) {
                    ...
                    // Falha silenciosa no backend, mas sucesso local
                    console.warn('Backend save failed/ignored:', data);
                    alert('âœ… ConfiguraÃ§Ãµes salvas (Local)!');
                }
            })
            .catch(err => {
...
function loadStoreConfig() {
    console.log('ðŸ”„ Carregando configuraÃ§Ãµes da loja...');
    if (!window.storeConfig) window.storeConfig = {};

    // 1. Tentar objeto JSON 'storeConfig'
    const localStoreConfigStr = localStorage.getItem('storeConfig');
    if (localStoreConfigStr) {
        try {
            const parsed = JSON.parse(localStoreConfigStr);
            if (parsed.storePixKey) window.storeConfig.storePixKey = parsed.storePixKey;
        } catch (e) { console.error('Error parsing storeConfig', e); }
    }

    // 2. Tentar chave simples 'storePixKey' (Legacy/Backup)
    const legacyKey = localStorage.getItem('storePixKey');
    if (legacyKey) {
        window.storeConfig.storePixKey = legacyKey;
    }

    // 3. Preencher input na tela de ConfiguraÃ§Ã£o
    const input = document.getElementById('store-pix-key');
    if (input && window.storeConfig.storePixKey) {
        input.value = window.storeConfig.storePixKey;
        console.log('âœ… Input preenchido com chave:', window.storeConfig.storePixKey);
    }
}

 
 / /   = = = = = = = = = = = = = = = = = = = =   O R D E R   N O T I F I C A T I O N   S Y S T E M   = = = = = = = = = = = = = = = = = = = = 
 
 
 
 l e t   p e n d i n g O r d e r   =   n u l l ; 
 
 l e t   n o t i f i c a t i o n A u d i o   =   n e w   A u d i o ( ' h t t p s : / / a s s e t s . m i x k i t . c o / a c t i v e _ s t o r a g e / s f x / 2 8 6 9 / 2 8 6 9 - p r e v i e w . m p 3 ' ) ;   / /   S o u n d   e f f e c t 
 
 
 
 f u n c t i o n   s t a r t O n l i n e O r d e r s P o l l i n g ( )   { 
 
         / /   C h e c k   e v e r y   1 0   s e c o n d s 
 
         s e t I n t e r v a l ( c h e c k N e w O r d e r s ,   1 0 0 0 0 ) ; 
 
         c h e c k N e w O r d e r s ( ) ;   / /   I n i t i a l   c h e c k 
 
 } 
 
 
 
 f u n c t i o n   c h e c k N e w O r d e r s ( )   { 
 
         / /   O n l y   c h e c k   i f   w e   a r e   i n   P O S   m o d e   ( L o g g e d   i n   a s   S t o r e / A d m i n ) 
 
...
 
 
         f e t c h ( ' / a p i / p o s / o n l i n e - o r d e r s ' ,   { 
 
...
                 . t h e n ( o r d e r s   = >   { 
 
                         i f   ( o r d e r s   & &   o r d e r s . l e n g t h   >   0 )   { 
 
                                 / /   F o u n d   p e n d i n g   o r d e r s ! 
 
                                 / /   S h o w   n o t i f i c a t i o n   f o r   t h e   O L D E S T   p e n d i n g   o r d e r   f i r s t 
 
                                 c o n s t   o l d e s t O r d e r   =   o r d e r s [ 0 ] ; 
 
 
 
                                 / /   O n l y   n o t i f y   i f   w e   a r e   n o t   a l r e a d y   s h o w i n g   t h i s   o r d e r 
 
                                 i f   ( ! p e n d i n g O r d e r   | |   p e n d i n g O r d e r . i d   ! = =   o l d e s t O r d e r . i d )   { 
 
                                         p e n d i n g O r d e r   =   o l d e s t O r d e r ; 
 
                                         s h o w O r d e r N o t i f i c a t i o n ( ) ; 
 
                                 } 
 
                         }   e l s e   { 
 
                                 / /   N o   p e n d i n g   o r d e r s 
 
                                 h i d e O r d e r N o t i f i c a t i o n ( ) ; 
 
                                 p e n d i n g O r d e r   =   n u l l ; 
 
                         } 
 
                 } ) 
 
                 . c a t c h ( e r r   = >   c o n s o l e . e r r o r ( ' E r r o r   p o l l i n g   o r d e r s : ' ,   e r r ) ) ; 
 
 } 
 
 
 
 f u n c t i o n   s h o w O r d e r N o t i f i c a t i o n ( )   { 
 
         c o n s t   n o t i f   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' n e w - o r d e r - n o t i f i c a t i o n ' ) ; 
 
         i f   ( n o t i f )   { 
 
                 n o t i f . s t y l e . d i s p l a y   =   ' b l o c k ' ; 
 
                 t r y   { 
 
                         n o t i f i c a t i o n A u d i o . p l a y ( ) . c a t c h ( e   = >   c o n s o l e . l o g ( ' A u d i o   p l a y   f a i l e d   ( u s e r   i n t e r a c t i o n   n e e d e d   f i r s t ) ' ) ) ; 
 
                 }   c a t c h   ( e )   {   } 
 
         } 
...
 
 
 f u n c t i o n   h i d e O r d e r N o t i f i c a t i o n ( )   { 
 
         c o n s t   n o t i f   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' n e w - o r d e r - n o t i f i c a t i o n ' ) ; 
 
         i f   ( n o t i f )   n o t i f . s t y l e . d i s p l a y   =   ' n o n e ' ; 
 
 } 
 
 
 
 f u n c t i o n   o p e n O r d e r A p p r o v a l M o d a l ( )   { 
 
         i f   ( ! p e n d i n g O r d e r )   r e t u r n ; 
 
 
 
         h i d e O r d e r N o t i f i c a t i o n ( ) ;   / /   H i d e   n o t i f i c a t i o n   w h i l e   v i e w i n g 
 
 
 
         c o n s t   m o d a l   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' o r d e r - a p p r o v a l - m o d a l ' ) ; 
 
         c o n s t   p r o o f I m g   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' a p p r o v a l - p r o o f - i m a g e ' ) ; 
 
         c o n s t   n o P r o o f   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' a p p r o v a l - n o - p r o o f ' ) ; 
 
         c o n s t   c l i e n t N a m e   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' a p p r o v a l - c l i e n t - n a m e ' ) ; 
 
         c o n s t   t o t a l   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' a p p r o v a l - o r d e r - t o t a l ' ) ; 
 
         c o n s t   i t e m s L i s t   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' a p p r o v a l - i t e m s - l i s t ' ) ; 
 
 
 
         / /   P o p u l a t e   d a t a 
 
         c l i e n t N a m e . t e x t C o n t e n t   =   p e n d i n g O r d e r . c l i e n t N a m e   | |   ' C l i e n t e ' ; 
 
         t o t a l . t e x t C o n t e n t   =   f o r m a t C u r r e n c y ( p e n d i n g O r d e r . t o t a l ) ; 
 
 
 
         / /   P r o o f   I m a g e 
 
         i f   ( p e n d i n g O r d e r . p r o o f I m a g e )   { 
 
                 p r o o f I m g . s r c   =   p e n d i n g O r d e r . p r o o f I m a g e ; 
 
                 p r o o f I m g . s t y l e . d i s p l a y   =   ' b l o c k ' ; 
 
                 n o P r o o f . s t y l e . d i s p l a y   =   ' n o n e ' ; 
 
         }   e l s e   { 
 
                 p r o o f I m g . s t y l e . d i s p l a y   =   ' n o n e ' ; 
 
                 n o P r o o f . s t y l e . d i s p l a y   =   ' b l o c k ' ; 
 
         } 
 
 
...
         / /   I t e m s 
 
         i t e m s L i s t . i n n e r H T M L   =   ' ' ; 
 
         i f   ( p e n d i n g O r d e r . i t e m s )   { 
 
                 p e n d i n g O r d e r . i t e m s . f o r E a c h ( i t e m   = >   { 
 
                         c o n s t   l i   =   d o c u m e n t . c r e a t e E l e m e n t ( ' l i ' ) ; 
 
                         l i . s t y l e . b o r d e r B o t t o m   =   ' 1 p x   s o l i d   r g b a ( 2 5 5 , 2 5 5 , 2 5 5 , 0 . 0 5 ) ' ; 
 
                         l i . s t y l e . p a d d i n g   =   ' 1 0 p x   0 ' ; 
 
                         l i . i n n e r H T M L   =   ` 
 
                                 < d i v   s t y l e = " d i s p l a y :   f l e x ;   j u s t i f y - c o n t e n t :   s p a c e - b e t w e e n ; " > 
 
                                         < s p a n > $ { i t e m . q u a n t i t y } x   $ { i t e m . n a m e } < / s p a n > 
 
                                         < s p a n   s t y l e = " f o n t - w e i g h t :   b o l d ; " > $ { f o r m a t C u r r e n c y ( i t e m . p r i c e   *   i t e m . q u a n t i t y ) } < / s p a n > 
 
                                 < / d i v > 
 
                         ` ; 
 
                         i t e m s L i s t . a p p e n d C h i l d ( l i ) ; 
 
                 } ) ; 
 
         } 
...
 
 
 f u n c t i o n   c l o s e O r d e r A p p r o v a l M o d a l ( )   { 
 
         d o c u m e n t . g e t E l e m e n t B y I d ( ' o r d e r - a p p r o v a l - m o d a l ' ) . s t y l e . d i s p l a y   =   ' n o n e ' ; 
 
         / /   R e - s h o w   n o t i f i c a t i o n   i f   o r d e r   s t i l l   p e n d i n g ?   
 
         / /   O r   a s s u m e   u s e r   w a n t s   t o   i g n o r e   i t   f o r   a   b i t 
 
         s e t T i m e o u t ( s h o w O r d e r N o t i f i c a t i o n ,   5 0 0 0 ) ;   / /   R e m i n d   i n   5 s   i f   s t i l l   p e n d i n g 
 
 } 
 
 
 
 f u n c t i o n   c o n f i r m C u r r e n t O r d e r ( )   { 
 
         i f   ( ! p e n d i n g O r d e r )   r e t u r n ; 
 
 
 
         i f   ( ! c o n f i r m ( ' C o n f i r m a r   p a g a m e n t o   e   f i n a l i z a r   p e d i d o ? ' ) )   r e t u r n ; 
 
 
 
         c o n s t   t o k e n   =   l o c a l S t o r a g e . g e t I t e m ( ' a u t h T o k e n ' ) ; 
 
         f e t c h ( ` / a p i / p o s / o n l i n e - o r d e r s / $ { p e n d i n g O r d e r . i d } / c o n f i r m ` ,   { 
 
                 m e t h o d :   ' P O S T ' , 
 
                 h e a d e r s :   {   ' A u t h o r i z a t i o n ' :   ` B e a r e r   $ { t o k e n } `   } 
...
                 . t h e n ( d a t a   = >   { 
 
                         a l e r t ( ' â S&   P e d i d o   A p r o v a d o ! ' ) ; 
 
                         d o c u m e n t . g e t E l e m e n t B y I d ( ' o r d e r - a p p r o v a l - m o d a l ' ) . s t y l e . d i s p l a y   =   ' n o n e ' ; 
 
                         p e n d i n g O r d e r   =   n u l l ;   / /   C l e a r   c u r r e n t 
 
                         c h e c k N e w O r d e r s ( ) ;   / /   C h e c k   f o r   n e x t   o n e 
 
                         i f   ( t y p e o f   l o a d S a l e s   = = =   ' f u n c t i o n ' )   l o a d S a l e s ( ) ;   / /   R e f r e s h   s a l e s   l i s t   i f   v i s i b l e 
 
                 } ) 
 
                 . c a t c h ( e r r   = >   a l e r t ( ' E r r o   a o   c o n f i r m a r   p e d i d o ' ) ) ; 
 
 } 
 
 
 
 f u n c t i o n   r e j e c t C u r r e n t O r d e r ( )   { 
 
         i f   ( ! p e n d i n g O r d e r )   r e t u r n ; 
 
 
 
         i f   ( ! c o n f i r m ( ' R e c u s a r   e s t e   p e d i d o ?   O   c l i e n t e   s e r Ã ¡   n o t i f i c a d o   ( f u t u r a m e n t e ) . ' ) )   r e t u r n ; 
 
 
 
         c o n s t   t o k e n   =   l o c a l S t o r a g e . g e t I t e m ( ' a u t h T o k e n ' ) ; 
 
         f e t c h ( ` / a p i / p o s / o n l i n e - o r d e r s / $ { p e n d i n g O r d e r . i d } / r e j e c t ` ,   { 
 
                 m e t h o d :   ' P O S T ' , 
 
                 h e a d e r s :   {   ' A u t h o r i z a t i o n ' :   ` B e a r e r   $ { t o k e n } `   } 
...
                 . t h e n ( d a t a   = >   { 
 
                         a l e r t ( ' P e d i d o   r e j e i t a d o . ' ) ; 
 
                         d o c u m e n t . g e t E l e m e n t B y I d ( ' o r d e r - a p p r o v a l - m o d a l ' ) . s t y l e . d i s p l a y   =   ' n o n e ' ; 
 
                         p e n d i n g O r d e r   =   n u l l ; 
 
                         c h e c k N e w O r d e r s ( ) ; 
 
                 } ) 
 
                 . c a t c h ( e r r   = >   a l e r t ( ' E r r o   a o   r e c u s a r   p e d i d o ' ) ) ; 
 
 } 
 
 
